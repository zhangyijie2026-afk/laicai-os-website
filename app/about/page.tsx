...
export default function About() {
  return <>
    ...
    <section id="principles" className="section shell"><div className="section-heading"><div className="section-label">OPERATING PRINCIPLES</div><h2>How the system is designed.</h2></div><div className="numbered-list">{principles.map(([title, copy], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{title}</h3><p>{copy}</p></div></article>)}</div></section>
